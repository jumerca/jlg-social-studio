...
function posterOrder(){
  ...
  function capture(){dialog?.box.querySelectorAll('input,textarea,select').forEach(el=>{if(el.name)state[el.name]=el.type==='checkbox'?el.checked:el.value})}
  function draw(){
    const type=currentType(),fmt=currentFormat();
    let body='';
    if(step===1)body=`<h3>Vos coordonnées</h3><p>Pour préparer la proposition et vous recontacter.</p><div class="formGrid"><label>Nom / prénom *<input name="name" required value="${esc(state.name)}"></label><label>Structure / activité *<input name="company" required value="${esc(state.company)}"></label><label>E-mail *<input name="email" type="email" required value="${esc(state.email)}"></label><label>Téléphone<input name="phone" value="${esc(state.phone)}"></label><label>Ville<input name="city" value="${esc(state.city)}"></label><label class="hp">Site web<input name="website" tabindex="-1"></label><label>Contact préféré<select name="contactPreference"><option>Email</option><option ${state.contactPreference==='Téléphone'?'selected':''}>Téléphone</option></select></label><label class="wide professionalConfirm"><input type="checkbox" name="professionalConfirmed" ${state.professionalConfirmed?'checked':''}><span>Je confirme agir dans le cadre de mon activité professionnelle ou pour le compte d’une entreprise, association, club, collectivité ou autre organisation. JLG Creative ne prend pas de commandes de particuliers à titre privé.</span></label></div>`;
    if(step===2)body=`<h3>Votre affiche</h3><p>Plus les indications sont précises, plus la première proposition sera proche de votre idée.</p><div class="formGrid posterFormGrid">
      ...
      <label>Usage professionnel<select name="usage"><option ${state.usage==='Communication professionnelle'?'selected':''}>Communication professionnelle</option><option ${state.usage==='Événement professionnel / associatif'?'selected':''}>Événement professionnel / associatif</option><option ${state.usage==='Décoration d’un lieu professionnel'?'selected':''}>Décoration d’un lieu professionnel</option><option ${state.usage==='Autre usage lié à la structure'?'selected':''}>Autre usage lié à la structure</option></select></label>
    ...
    dialog.box.querySelectorAll('input,textarea,select').forEach(el=>el.addEventListener('input',()=>{state[el.name]=el.type==='checkbox'?el.checked:el.value;if(el.name==='style'||el.name==='type'){capture();draw()}}));
    dialog.box.querySelector('#back')?.addEventListener('click',()=>{capture();step--;draw()});
    dialog.box.querySelector('#next')?.addEventListener('click',()=>{capture();if(step===1&&(!state.name.trim()||!state.company.trim()||!validEmail(state.email)))return showToast('Nom, structure et e-mail valide sont obligatoires.',true);if(step===1&&!state.professionalConfirmed)return showToast('Confirmez que la demande est faite dans un cadre professionnel ou pour une organisation.',true);if(step===2&&!state.subject.trim())return showToast('Précisez le sujet de l’affiche.',true);step++;draw()});
