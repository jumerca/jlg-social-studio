...
async function studioSystemNotification(title,body){if(!('Notification' in window)||Notification.permission!=='granted')return;const options={body,icon:new URL('./assets/icon-192.png',location.href).href,badge:new URL('./assets/icon-192.png',location.href).href,tag:'jlg-studio-new-order',data:{url:new URL('./index.html',location.href).href}};try{if('serviceWorker' in navigator){const reg=await navigator.serviceWorker.ready;await reg.showNotification(title,options);return}new Notification(title,options)}catch{}}
...
function renderLogin(){root.innerHTML=`<div class="loginPage"><section class="loginCard"><div class="studioLoginBrand"><img src="./assets/brand-mark.png" alt=""><span class="studioBrandDivider"></span><span><strong>JLG</strong><small>STUDIO</small></span></div><p>ESPACE PRIVÉ</p><h1>Pilotage & production</h1><label>Mot de passe<input id="pwd" type="password" autocomplete="current-password" autofocus></label><button class="primary full" id="login">Se connecter</button><button class="secondary full loginInstall" type="button" data-install-app>Installer JLG Studio</button><p class="installState" data-install-state></p><a href="../jlg-creative/">← Retour à JLG Creative</a><div id="loginErr" role="alert"></div></section></div>`;const go=async()=>{const password=document.querySelector('#pwd').value;const btn=document.querySelector('#login');btn.disabled=true;btn.textContent='Connexion…';try{const r=await fetch(API+'?action=login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password})});const j=await r.json().catch(()=>({}));if(!r.ok)throw Error(j.error||'Erreur');token=j.token;sessionStorage.setItem('jlg_token',token);await load();}catch(e){document.querySelector('#loginErr').textContent=e.message;btn.disabled=false;btn.textContent='Se connecter'}};document.querySelector('#login').onclick=go;document.querySelector('#pwd').onkeydown=e=>{if(e.key==='Enter')go()}}
async function load(){data=await api('bootstrap');announceNewOrders(data.orders||[],true);render();startStudioPolling()}
const labels={dashboard:'Tableau de bord',orders:'Demandes clients',packs:'Toutes mes offres',clients:'Clients',projects:'Projets',tasks:'Production',quotes:'Devis',invoices:'Finance',deliveries:'Livraisons',settings:'Réglages'};
function nav(){return `<aside class="studioSide"><div class="studioBrand studioBrandV15"><img src="./assets/brand-mark.png" alt=""><div><strong>Studio</strong><span>JLG Creative</span></div></div><nav>${[['dashboard','Tableau de bord'],['orders','Demandes'],['packs','Toutes mes offres'],['clients','Clients'],['projects','Projets'],['tasks','Production'],['quotes','Devis'],['invoices','Finance'],['deliveries','Livraisons'],['settings','Réglages']].map(([k,l])=>`<button data-view="${k}" class="${view===k?'active':''}">${l}</button>`).join('')}</nav><div class="sideFooter"><button class="sideInstall" type="button" data-install-app>Installer JLG Studio</button><p class="installState" data-install-state></p><a href="../jlg-creative/">Voir JLG Creative ↗</a><button id="logout" class="sideLogout">Déconnexion</button></div></aside>`}
const mobileMoreViews=['packs','clients','tasks','invoices','deliveries','settings'];
function mobileNav(){
  const primary=[['dashboard','⌂','Accueil'],['orders','◎','Demandes'],['projects','▣','Projets'],['quotes','▤','Devis']];
  const moreActive=mobileMoreViews.includes(view);
  return `<nav class="studioMobileNav" aria-label="Navigation JLG Studio">${primary.map(([k,icon,label])=>`<button type="button" data-mobile-view="${k}" class="${view===k?'active':''}"><span aria-hidden="true">${icon}</span><small>${label}</small></button>`).join('')}<button type="button" id="mobileMore" class="${moreActive?'active':''}" aria-expanded="false" aria-controls="mobileMoreSheet"><span aria-hidden="true">•••</span><small>Plus</small></button></nav>
  <div class="mobileMoreSheet" id="mobileMoreSheet" aria-hidden="true"><button class="mobileMoreBackdrop" type="button" aria-label="Fermer le menu"></button><section class="mobileMorePanel" role="dialog" aria-modal="true" aria-label="Toutes les rubriques"><div class="mobileMoreHandle"></div><div class="mobileMoreHead"><div><small>JLG STUDIO</small><strong>Toutes les rubriques</strong></div><button id="mobileMoreClose" type="button" aria-label="Fermer">×</button></div><button class="mobileInstallAction" type="button" data-install-app>Installer JLG Studio sur ce téléphone</button><p class="installState mobileInstallState" data-install-state></p><div class="mobileMoreGrid">${[['packs','Toutes mes offres'],['clients','Clients'],['tasks','Production'],['invoices','Finance'],['deliveries','Livraisons'],['settings','Réglages']].map(([k,l])=>`<button type="button" data-mobile-view="${k}" class="${view===k?'active':''}">${l}<span>›</span></button>`).join('')}</div><div class="mobileMoreLinks"><a href="../jlg-creative/">Ouvrir JLG Creative ↗</a><button id="mobileLogout" type="button">Déconnexion</button></div></section></div>`;
}
function createButton(){if(['clients','projects','tasks','quotes','invoices','deliveries','packs'].includes(view))return `<button class="primary" id="createRecord">+ Nouveau</button>`;return ''}
function render(){root.innerHTML=`<div class="studioShell">${nav()}<main class="studioMain"><header class="studioTop"><div class="studioTopTitle"><p>JLG STUDIO</p><h1>${labels[view]}</h1></div><div class="studioTopActions">${createButton()}<button class="studioNotifToggle" id="studioNotifToggle" title="Notifications" aria-label="Notifications">🔔<b id="studioNotifCount" hidden>0</b></button><button class="secondary" id="refresh" aria-label="Actualiser">Actualiser</button></div></header><section id="viewRoot"></section></main>${mobileNav()}</div>`;
const switchView=k=>{view=k;render();window.scrollTo({top:0,left:0,behavior:'auto'})};
document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>switchView(b.dataset.view));
document.querySelectorAll('[data-mobile-view]').forEach(b=>b.onclick=()=>switchView(b.dataset.mobileView));
const moreBtn=document.querySelector('#mobileMore'),moreSheet=document.querySelector('#mobileMoreSheet');
const setMore=open=>{if(!moreSheet||!moreBtn)return;moreSheet.classList.toggle('open',open);moreSheet.setAttribute('aria-hidden',String(!open));moreBtn.setAttribute('aria-expanded',String(open))};
moreBtn?.addEventListener('click',()=>setMore(!moreSheet.classList.contains('open')));
document.querySelector('#mobileMoreClose')?.addEventListener('click',()=>setMore(false));
document.querySelector('.mobileMoreBackdrop')?.addEventListener('click',()=>setMore(false));
document.querySelector('#refresh').onclick=()=>load().catch(e=>toast(e.message,true));
document.querySelector('#studioNotifToggle').onclick=async()=>{await enableStudioNotifications();switchView('orders')};
updateStudioNotifBadge();document.querySelector('#logout').onclick=logout;document.querySelector('#mobileLogout')?.addEventListener('click',logout);document.querySelector('#createRecord')?.addEventListener('click',()=>createForView());renderView();document.dispatchEvent(new CustomEvent('jlg:rendered'))}
function renderView(){const v=document.querySelector('#viewRoot');if(view==='dashboard')return renderDashboard(v);if(view==='orders')return renderOrders(v);if(view==='packs')return renderPacks(v);if(view==='clients')return renderClients(v);if(view==='projects')return renderProjects(v);if(view==='tasks')return renderTasks(v);if(view==='quotes')return renderQuotes(v);if(view==='invoices')return renderInvoices(v);if(view==='deliveries')return renderDeliveries(v);if(view==='settings')return renderSettings(v)}
...
function createForView(){if(view==='packs')return editPack(null);if(view==='clients')return editClient(null);if(view==='projects')return editProject(null);if(view==='tasks')return editTask(null);if(view==='quotes')return editQuote(null);if(view==='invoices')return editInvoice(null);if(view==='deliveries')return editDelivery(null)}
...
function editClient(c){const isNew=!c;const o=modal(isNew?'Nouveau client':'Modifier le client',`<div class="formGrid"><label>Nom / structure *<input name="name" value="${esc(c?.name||'')}"></label><label>Activité<input name="activity" value="${esc(c?.activity||'')}"></label><label>E-mail<input name="email" type="email" value="${esc(c?.email||'')}"></label><label>Téléphone<input name="phone" value="${esc(c?.phone||'')}"></label><label class="wide">Adresse<input name="address" value="${esc(c?.address||'')}"></label><label>Code postal<input name="postal_code" value="${esc(c?.postal_code||'')}"></label><label>Ville<input name="city" value="${esc(c?.city||'')}"></label><label>Pays<input name="country" value="${esc(c?.country||'France')}"></label><label>SIREN client (si pro)<input name="siren" value="${esc(c?.siren||'')}"></label><label>Statut<select name="status"><option>Actif</option><option ${c?.status==='Prospect'?'selected':''}>Prospect</option><option ${c?.status==='Inactif'?'selected':''}>Inactif</option></select></label><label class="wide">Notes<textarea name="notes" rows="5">${esc(c?.notes||'')}</textarea></label></div>`,`<button class="primary" id="save">Enregistrer</button>`);o.querySelector('#save').onclick=()=>saveForm(o,'clients',c,['name','activity','email','phone','address','postal_code','city','country','siren','status','notes'])}
...
function printQuote(q){
  const client=byId(data.clients,q.client_id), project=byId(data.projects,q.project_id), biz=data.businessProfile||{};
  const seller=[biz.publisher,biz.legal_form,biz.siret?('SIRET '+biz.siret):(biz.siren?('SIREN '+biz.siren):''),[biz.address,biz.postal_code,biz.city,biz.country].filter(Boolean).join(' '),biz.email,biz.phone,biz.vat_number?('TVA '+biz.vat_number):biz.vat_note].filter(Boolean).join(' · ');
  ...
  <div class="head"><div><h1>${esc(biz.display_name||'JLG Creative')}</h1><div class="muted">Communication · Design · Supports professionnels</div><div class="muted">${esc(seller)}</div></div><div><h2>Devis</h2><strong>${esc(q.reference)}</strong><div class="muted">Valable jusqu’au ${esc(q.valid_until||'—')}</div></div></div>
...
function editQuote(q){
  ...
  itemRoot.addEventListener('input',e=>{const i=Number(e.target.dataset.lineQty??e.target.dataset.linePrice??e.target.dataset.lineLabel??e.target.dataset.lineDesc);if(!Number.isInteger(i)||!items[i])return;if(e.target.dataset.lineQty!==undefined)items[i].qty=Number(e.target.value||0);if(e.target.dataset.linePrice!==undefined)items[i].unit_price=Number(e.target.value||0);if(e.target.dataset.lineLabel!==undefined)items[i].label=e.target.value;if(e.target.dataset.lineDesc!==undefined)items[i].description=e.target.value;items[i].total=Math.max(0,Number(items[i].qty||0))*Math.max(0,Number(items[i].unit_price||0));const line=e.target.closest('.quoteItem');const totalNode=line?.querySelector('.lineTotal strong');if(totalNode)totalNode.textContent=money(items[i].total);calc()});
...
function printInvoice(i){
  if(!i)return;const client=byId(data.clients,i.client_id),quote=byId(data.quotes,i.quote_id),items=(i.items&&i.items.length?i.items:(quote?.items||[])),payments=(data.payments||[]).filter(p=>p.invoice_id===i.id),biz=data.businessProfile||{},rem=Math.max(0,Number(i.total||0)-Number(i.paid_amount||0));
  const seller=[biz.publisher,biz.legal_form,biz.siret?('SIRET '+biz.siret):(biz.siren?('SIREN '+biz.siren):''),[biz.address,biz.postal_code,biz.city,biz.country].filter(Boolean).join(' '),biz.email,biz.phone,biz.vat_number?('TVA '+biz.vat_number):biz.vat_note].filter(Boolean).join(' · ');
  const rows=items.map(x=>'<tr><td><strong>'+esc(x.label||'Prestation')+'</strong><small>'+esc(x.description||'')+'</small></td><td>'+Number(x.qty||1)+'</td><td>'+money(x.unit_price)+'</td><td>'+money(x.total)+'</td></tr>').join('');
  const w=window.open('','_blank','width=1000,height=850');if(!w)return toast('Autorisez les fenêtres pour afficher la facture.',true);
  w.document.write('<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>'+esc(i.reference)+'</title><style>body{font-family:Arial,sans-serif;color:#173042;max-width:900px;margin:32px auto;padding:0 22px}h1,h2{font-family:Georgia,serif}.head{display:flex;justify-content:space-between;gap:30px;border-bottom:3px solid #c79b45;padding-bottom:16px}.muted{color:#6d7b84;font-size:11px;line-height:1.45}.client{background:#f5f2eb;padding:14px;border-radius:12px;margin:20px 0}table{width:100%;border-collapse:collapse;font-size:12px}th,td{padding:10px;border-bottom:1px solid #ddd;text-align:left;vertical-align:top}th{text-transform:uppercase;font-size:9px;color:#78858c}td small{display:block;color:#78858c}.totals{margin-left:auto;width:350px;margin-top:18px}.totals div{display:flex;justify-content:space-between;padding:6px}.grand{font-size:18px;font-weight:bold;border-top:2px solid #173042}.payments{margin-top:20px;background:#edf4ee;padding:12px;border-radius:10px}.note{margin-top:20px;background:#f7f5f0;padding:12px;border-radius:10px;white-space:pre-wrap}@media print{body{margin:0}}</style></head><body><div class="head"><div><h1>'+esc(biz.display_name||'JLG Creative')+'</h1><div class="muted">'+esc(seller)+'</div></div><div><h2>'+esc(i.kind||'Facture')+'</h2><strong>'+esc(i.reference)+'</strong><div class="muted">Émise le '+esc(i.issue_date||'—')+' · Échéance '+esc(i.due_date||'—')+'</div></div></div><div class="client"><strong>'+esc(client?.name||'Client')+'</strong><div>'+esc([client?.address,[client?.postal_code,client?.city,client?.country].filter(Boolean).join(' '),client?.email,client?.phone,client?.siren?('SIREN '+client.siren):''].filter(Boolean).join(' · '))+'</div></div><div class="muted">Date de prestation / acompte : '+esc(i.service_date||i.issue_date||'—')+(i.purchase_order_reference?' · Bon de commande : '+esc(i.purchase_order_reference):'')+'</div><table><thead><tr><th>Prestation</th><th>Qté</th><th>Prix unit.</th><th>Total</th></tr></thead><tbody>'+(rows||'<tr><td colspan="4">Détail non renseigné</td></tr>')+'</tbody></table><div class="totals">'+(Number(i.gross_total||0)>Number(i.total||0)?'<div><span>Montant du devis</span><strong>'+money(i.gross_total)+'</strong></div>':'')+(Number(i.deduction_amount||0)>0?'<div><span>Acompte déduit</span><strong>- '+money(i.deduction_amount)+'</strong></div>':'')+'<div class="grand"><span>Montant facturé</span><strong>'+money(i.total)+'</strong></div><div><span>Encaissé</span><strong>'+money(i.paid_amount)+'</strong></div><div><span>Reste dû</span><strong>'+money(rem)+'</strong></div></div>'+(payments.length?'<div class="payments"><strong>Règlements</strong><ul>'+payments.map(p=>'<li>'+new Date(p.paid_at).toLocaleDateString('fr-FR')+' · '+money(p.amount)+(p.method?' · '+esc(p.method):'')+'</li>').join('')+'</ul></div>':'')+(i.notes?'<div class="note">'+esc(i.notes)+'</div>':'')+'<div class="note">'+esc([biz.operation_type,biz.vat_number?('TVA '+biz.vat_number):biz.vat_note,biz.payment_terms_note,biz.early_payment_discount_note,biz.late_payment_note].filter(Boolean).join('\n'))+'</div><script>window.onload=()=>window.print()<\/script></body></html>');w.document.close();
}
function invoiceDetail(i){
  if(!i)return;
  const quote=byId(data.quotes,i.quote_id), items=(i.items&&i.items.length?i.items:(quote?.items||[])), payments=(data.payments||[]).filter(p=>p.invoice_id===i.id), rem=Math.max(0,Number(i.total||0)-Number(i.paid_amount||0));
  const lines=items.length?'<div class="deliver">'+items.map(x=>'<div><span>✓</span><span><strong>'+esc(x.label||'Prestation')+'</strong> · '+Number(x.qty||1)+' × '+money(x.unit_price)+' = '+money(x.total)+'</span></div>').join('')+'</div>':'<p class="muted">Aucun détail de ligne disponible.</p>';
  const pays=payments.length?'<div class="adminOptions"><h3>Règlements</h3><ul>'+payments.map(p=>'<li>'+new Date(p.paid_at).toLocaleDateString('fr-FR')+' · <strong>'+money(p.amount)+'</strong>'+(p.method?' · '+esc(p.method):'')+(p.reference?' · réf. '+esc(p.reference):'')+'</li>').join('')+'</ul></div>':'';
  modal(esc(i.kind||'Facture')+' · '+esc(i.reference),'<div class="detailGrid"><div><small>Statut</small><strong>'+esc(i.status)+'</strong></div><div><small>Montant</small><strong>'+money(i.total)+'</strong></div><div><small>Encaissé</small><strong>'+money(i.paid_amount)+'</strong></div><div><small>Reste dû</small><strong>'+money(rem)+'</strong></div></div><h3>Détail</h3>'+lines+pays+(i.notes?'<div class="adminOptions"><h3>Conditions / notes</h3><p>'+esc(i.notes)+'</p></div>':''));
}
function renderInvoices(v){const billed=data.invoices.reduce((s,i)=>s+Number(i.total||0),0),paid=data.invoices.reduce((s,i)=>s+Number(i.paid_amount||0),0);v.innerHTML=`<div class="financeKpis"><div><span>Facturé</span><strong>${money(billed)}</strong></div><div><span>Encaissé</span><strong>${money(paid)}</strong></div><div><span>À encaisser</span><strong>${money(Math.max(0,billed-paid))}</strong></div></div><div class="simpleList richList">${data.invoices.length?data.invoices.map(i=>`<article><div><strong>${esc(i.reference)}</strong><small>${esc(i.kind||'Facture')} · ${esc(clientName(i.client_id))}${i.project_id?' · '+esc(projectName(i.project_id)):''}</small></div><span class="status">${esc(i.status)}</span><span>${money(i.total)}</span><span>Payé ${money(i.paid_amount)}</span><div class="listActions"><button class="secondary" data-detailinvoice="${i.id}">Détail</button><button class="secondary" data-printinvoice="${i.id}">PDF</button><button class="secondary" data-editinvoice="${i.id}">Modifier</button>${Number(i.paid_amount||0)<Number(i.total||0)?`<button class="primary mini" data-paidinvoice="${i.id}">Encaisser</button>`:''}<button class="dangerBtn" data-deleteinvoice="${i.id}">Supprimer</button></div></article>`).join(''):empty('Aucune facture.')}</div>`;document.querySelectorAll('[data-detailinvoice]').forEach(b=>b.onclick=()=>invoiceDetail(byId(data.invoices,b.dataset.detailinvoice)));document.querySelectorAll('[data-printinvoice]').forEach(b=>b.onclick=()=>printInvoice(byId(data.invoices,b.dataset.printinvoice)));document.querySelectorAll('[data-editinvoice]').forEach(b=>b.onclick=()=>editInvoice(byId(data.invoices,b.dataset.editinvoice)));document.querySelectorAll('[data-paidinvoice]').forEach(b=>b.onclick=()=>markPaid(byId(data.invoices,b.dataset.paidinvoice)));document.querySelectorAll('[data-deleteinvoice]').forEach(b=>b.onclick=()=>removeRecord('invoices',byId(data.invoices,b.dataset.deleteinvoice),'facture'))}
function editInvoice(i){
  const isNew=!i;
  const o=modal(isNew?'Nouvelle facture':'Modifier la facture',`<div class="formGrid"><label>Référence<input name="reference" value="${esc(i?.reference||'')}" placeholder="Attribuée automatiquement si vide"></label><label>Type<select name="kind">${['Facture','Acompte','Solde'].map(x=>`<option ${i?.kind===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Statut<select name="status">${['Brouillon','Envoyée','Partiellement payée','Payée','Échue'].map(x=>`<option ${i?.status===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Client<select name="client_id">${opts(data.clients,i?.client_id)}</select></label><label>Projet<select name="project_id">${opts(data.projects,i?.project_id)}</select></label><label>Total à régler (€)<input name="total" type="number" min="0" step="0.01" value="${Number(i?.total||0)}"></label><label>Date d'émission<input name="issue_date" type="date" value="${esc(i?.issue_date||today())}"></label><label>Date de prestation / acompte<input name="service_date" type="date" value="${esc(i?.service_date||i?.issue_date||today())}"></label><label>Échéance<input name="due_date" type="date" value="${esc(i?.due_date||'')}"></label><label>Bon de commande<input name="purchase_order_reference" value="${esc(i?.purchase_order_reference||'')}"></label><label class="wide">Notes / conditions<textarea name="notes" rows="4">${esc(i?.notes||'')}</textarea></label></div>`,`<button class="primary" id="save">Enregistrer</button>`);
  o.querySelector('#save').onclick=()=>saveForm(o,'invoices',i,['reference','kind','status','client_id','project_id','total','issue_date','service_date','due_date','purchase_order_reference','notes'],['total'])
}
async function markPaid(i){
  if(!i)return;
  const rem=Math.max(0,Number(i.total||0)-Number(i.paid_amount||0));
  if(rem<=0)return toast('Cette facture est déjà réglée.');
  const o=modal('Enregistrer un règlement',`<div class="formGrid"><label>Facture<input value="${esc(i.reference)}" disabled></label><label>Reste à encaisser<input value="${money(rem)}" disabled></label><label>Montant (€)<input name="amount" type="number" min="0.01" max="${rem}" step="0.01" value="${rem}"></label><label>Date<input name="paid_at" type="datetime-local" value="${new Date().toISOString().slice(0,16)}"></label><label>Moyen de paiement<select name="method"><option>Virement</option><option>Carte bancaire</option><option>Chèque</option><option>Espèces</option><option>Autre</option></select></label><label>Référence<input name="reference" placeholder="Réf. virement, chèque…"></label><label class="wide">Note<textarea name="notes" rows="3"></textarea></label></div>`,`<button class="primary" id="savePayment">Valider l’encaissement</button>`);
  o.querySelector('#savePayment').onclick=async()=>{const q=n=>o.querySelector(`[name="${n}"]`)?.value||'';const amount=Number(q('amount'));if(!amount||amount<=0||amount>rem+0.009)return toast('Montant de règlement invalide.',true);try{await api('record-payment',{method:'POST',body:JSON.stringify({invoice_id:i.id,amount,paid_at:q('paid_at')?new Date(q('paid_at')).toISOString():new Date().toISOString(),method:q('method'),reference:q('reference'),notes:q('notes')})});o.remove();toast('Règlement enregistré.');await load()}catch(e){toast(e.message,true)}}
}
function projectPaymentState(projectId){
  const invoices=(data.invoices||[]).filter(i=>i.project_id===projectId),accepted=(data.quotes||[]).filter(q=>q.project_id===projectId&&q.status==='Accepté');
  const outstanding=invoices.reduce((s,i)=>s+Math.max(0,Number(i.total||0)-Number(i.paid_amount||0)),0);
  const clear=accepted.length>0&&invoices.length>0&&outstanding<0.01;
  const reason=!accepted.length?'Aucun devis accepté.':!invoices.length?'Aucune facture émise.':outstanding>=0.01?'Il reste '+money(outstanding)+' à encaisser.':'Tous les règlements sont encaissés.';
  return {clear,outstanding,reason,accepted:accepted.length,invoices:invoices.length};
}
function renderDeliveries(v){
  const rows=data.deliveries||[];v.innerHTML=`<div class="studioIntro"><div><strong>Livraisons client</strong><p>Publie les fichiers finaux seulement quand le circuit devis et règlement est cohérent.</p></div><div><b>${rows.length}</b><span>livraison${rows.length!==1?'s':''}</span></div></div><div class="simpleList richList">${rows.length?rows.map(d=>`<article><div><strong>${esc(d.title)}</strong><small>${esc(projectName(d.project_id))} · ${esc(d.version||'V1')}</small></div><span class="status">${esc(d.status)}</span><span>${esc(d.delivery_date||'—')}</span><span>${d.url?'Lien prêt':'Sans lien'}</span><div class="listActions"><button class="secondary" data-editdelivery="${d.id}">Modifier</button>${d.url?`<a class="secondary" target="_blank" rel="noopener" href="${esc(d.url)}">Ouvrir ↗</a>`:''}<button class="dangerBtn" data-deletedelivery="${d.id}">Supprimer</button></div></article>`).join(''):empty('Aucune livraison publiée.')}</div>`;document.querySelectorAll('[data-editdelivery]').forEach(b=>b.onclick=()=>editDelivery(byId(data.deliveries,b.dataset.editdelivery)));document.querySelectorAll('[data-deletedelivery]').forEach(b=>b.onclick=()=>removeRecord('deliveries',byId(data.deliveries,b.dataset.deletedelivery),'livraison'))
}
function editDelivery(d){
  const isNew=!d,o=modal(isNew?'Nouvelle livraison':'Modifier la livraison',`<div class="formGrid"><label class="wide">Titre *<input name="title" value="${esc(d?.title||'Livraison finale')}"></label><label>Projet *<select name="project_id">${opts(data.projects,d?.project_id)}</select></label><label>Version<input name="version" value="${esc(d?.version||'V1')}"></label><label>Statut<select name="status">${['À livrer','Disponible','Livré'].map(x=>`<option ${d?.status===x?'selected':''}>${x}</option>`).join('')}</select></label><label>Date de livraison<input name="delivery_date" type="date" value="${esc(d?.delivery_date||today())}"></label><label class="wide">Lien du fichier / dossier<input name="url" type="url" value="${esc(d?.url||'')}" placeholder="https://..."></label><div class="wide deliveryGate" id="deliveryGate"></div><label class="wide deliveryOverride"><input id="overridePayment" type="checkbox"> Autoriser exceptionnellement la publication malgré le blocage financier</label></div>`,`<button class="primary" id="saveDelivery">Enregistrer la livraison</button>`);
  const project=o.querySelector('[name=project_id]'),status=o.querySelector('[name=status]'),gate=o.querySelector('#deliveryGate'),override=o.querySelector('#overridePayment');
  const refreshGate=()=>{const s=projectPaymentState(project.value),final=['Disponible','Livré'].includes(status.value);gate.className='wide deliveryGate '+(s.clear?'ok':final?'blocked':'info');gate.innerHTML=s.clear?'✓ Paiement OK — publication finale autorisée.':(final?'⚠ '+esc(s.reason)+' La publication sera bloquée sauf dérogation.':'ℹ '+esc(s.reason));override.closest('label').hidden=s.clear||!final};project.onchange=refreshGate;status.onchange=refreshGate;refreshGate();
  o.querySelector('#saveDelivery').onclick=async()=>{const q=n=>o.querySelector(`[name="${n}"]`)?.value||'',payload={id:d?.id||'',title:q('title'),project_id:q('project_id'),version:q('version'),status:q('status'),delivery_date:q('delivery_date'),url:q('url'),override_payment:override.checked};if(!payload.title.trim()||!payload.project_id)return toast('Titre et projet obligatoires.',true);if(['Disponible','Livré'].includes(payload.status)&&!payload.url.trim())return toast('Ajoute le lien de livraison avant publication.',true);try{await api('save-delivery',{method:isNew?'POST':'PATCH',body:JSON.stringify(payload)});o.remove();toast(payload.status==='Livré'?'Projet livré et clôturé.':'Livraison enregistrée.');await load()}catch(e){toast(e.message,true)}}
}
function renderSettings(v){
  const b=data.businessProfile||{}, missing=[['publisher','nom / responsable'],['address','adresse'],['postal_code','code postal'],['city','ville'],['email','e-mail'],['phone','téléphone'],['siren','SIREN / SIRET']].filter(([k])=>!String(b[k]||b[k==='siren'?'siret':k]||'').trim()).map(x=>x[1]);
  v.innerHTML=`<div class="settingsGrid"><article class="settingsWide"><h3>Identité légale JLG Creative</h3><p>Ces informations alimentent les documents commerciaux et la page légale. JLG Creative est actuellement réservé aux professionnels et organisations ; la médiation de la consommation n’est donc pas utilisée dans ce périmètre. ${missing.length?'<strong class="legalWarning">À compléter : '+esc(missing.join(', '))+'.</strong>':'<strong class="doneText">✓ Profil légal complet.</strong>'}</p><div class="formGrid"><label>Nom commercial<input data-biz="display_name" value="${esc(b.display_name||'JLG Creative')}"></label><label>Responsable / nom légal<input data-biz="publisher" value="${esc(b.publisher||'')}"></label><label>Forme juridique<input data-biz="legal_form" value="${esc(b.legal_form||'')}" placeholder="EI, micro-entrepreneur, SAS…"></label><label>SIREN<input data-biz="siren" value="${esc(b.siren||'')}"></label><label>SIRET<input data-biz="siret" value="${esc(b.siret||'')}"></label><label>N° TVA<input data-biz="vat_number" value="${esc(b.vat_number||'')}"></label><label class="wide">Mention TVA<input data-biz="vat_note" value="${esc(b.vat_note||'')}" placeholder="Ex. TVA non applicable, art. 293 B du CGI"></label><label class="wide">Adresse<input data-biz="address" value="${esc(b.address||'')}"></label><label>Code postal<input data-biz="postal_code" value="${esc(b.postal_code||'')}"></label><label>Ville<input data-biz="city" value="${esc(b.city||'')}"></label><label>Pays<input data-biz="country" value="${esc(b.country||'France')}"></label><label>E-mail<input data-biz="email" type="email" value="${esc(b.email||'')}"></label><label>Téléphone<input data-biz="phone" value="${esc(b.phone||'')}"></label><label>Médiateur consommation (si ouverture aux particuliers)<input data-biz="mediator_name" value="${esc(b.mediator_name||'')}"></label><label>URL médiateur (si ouverture aux particuliers)<input data-biz="mediator_url" value="${esc(b.mediator_url||'')}"></label><label>Nature des opérations<input data-biz="operation_type" value="${esc(b.operation_type||'Prestations de services')}"></label><label class="wide">Conditions de paiement<input data-biz="payment_terms_note" value="${esc(b.payment_terms_note||'Paiement à la date d’échéance indiquée sur la facture.')}"></label><label class="wide">Escompte anticipé<input data-biz="early_payment_discount_note" value="${esc(b.early_payment_discount_note||'Escompte pour paiement anticipé : néant.')}"></label><label class="wide">Retard / recouvrement<textarea data-biz="late_payment_note" rows="3">${esc(b.late_payment_note||'Pour les clients professionnels : pénalités de retard au taux de refinancement semestriel de la BCE en vigueur majoré de 10 points ; indemnité forfaitaire de 40 € pour frais de recouvrement.')}</textarea></label></div><button class="primary" id="saveBusiness">Enregistrer l’identité légale</button></article><article><h3>Sécurité du Studio</h3><p>Changer le mot de passe déconnecte les anciennes sessions.</p><label>Nouveau mot de passe<input id="newPassword" type="password" autocomplete="new-password" placeholder="10 caractères minimum"></label><button class="primary" id="changePassword">Changer le mot de passe</button></article><article><h3>Application</h3><p>JLG Creative utilise Supabase pour les données et une PWA installable pour les clients.</p><button class="secondary" id="settingsLogout">Déconnexion</button></article></div>`;
  document.querySelector('#settingsLogout').onclick=logout;
  document.querySelector('#saveBusiness').onclick=async()=>{const profile={};document.querySelectorAll('[data-biz]').forEach(el=>profile[el.dataset.biz]=el.value);try{await api('update-business-profile',{method:'POST',body:JSON.stringify({profile})});toast('Identité légale enregistrée.');await load()}catch(e){toast(e.message,true)}};
