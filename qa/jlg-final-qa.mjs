...
const fakeData={
 orders:[
  ...
  {id:'o2',reference:'CMD-QA-002',status:'Accusée',pack_id:null,pack_name:'Affiche personnalisée · A4 · Ville / voyage',pack_price:6.9,estimate:6.9,name:'Client Affiche',company:'',email:'poster@example.com',phone:'',city:'Toulon',objective:'Affiche Toulon',deadline:'2026-10-12',budget:'6,90 €',style:'Vintage minimaliste',notes:'',target_audience:'',key_message:'TOULON',existing_assets:'',content_status:'',constraints:'',inspiration:'',requested_supports:'A4 · Portrait',contact_preference:'Email',options:[],created_at:'2026-09-25T08:02:00Z',poster_details:{type:'Ville / voyage',subject:'Toulon',style:'Vintage minimaliste',format:'A4 · 21 × 29,7 cm',orientation:'Portrait',title:'TOULON',colors:'Bleu',elements_include:'Port',elements_avoid:'',usage:'Communication professionnelle',signature:true}}
 ],
 packs:fakePacks,
 clients:[{id:'c1',name:'Client QA',activity:'Commerce',email:'qa@example.com',phone:'0600000000',address:'2 avenue QA',postal_code:'19100',city:'Brive',country:'France',siren:'987654321',status:'Actif',notes:''}],
 projects:[{id:'pr1',name:'Projet QA',client_id:'c1',budget:349,status:'En cours',progress:40,deadline:'2026-10-20',objective:'Objectif QA',brief:'Brief QA',review_token:'review-qa'},{id:'pr2',name:'Projet payé',client_id:'c1',budget:100,status:'À valider',progress:90,deadline:'2026-10-22',objective:'Livraison QA',brief:'Brief payé',review_token:'review-paid'}],
 ...
 quotes:[{id:'q1',reference:'DEV-QA-001',status:'Brouillon',client_id:'c1',project_id:'pr1',subtotal:90,discount_percent:0,discount_amount:0,deposit_percent:30,deposit_amount:27,total:90,valid_until:'2026-10-25',notes:'Conditions QA',items:[{id:'li1',label:'Flyer simple',description:'Flyer QA',category:'Supports imprimés',unit:'support',qty:1,unit_price:90,total:90}]},{id:'q2',reference:'DEV-QA-002',status:'Accepté',client_id:'c1',project_id:'pr2',subtotal:100,discount_percent:0,discount_amount:0,deposit_percent:0,deposit_amount:0,total:100,valid_until:'2026-10-25',notes:'Payé',items:[{id:'li2',label:'Livrable final',description:'QA',category:'Digital',unit:'prestation',qty:1,unit_price:100,total:100}]}],
 invoices:[{id:'i1',reference:'FAC-QA-001',kind:'Acompte',status:'Partiellement payée',client_id:'c1',project_id:'pr1',quote_id:'q1',items:[{id:'li1',label:'Flyer simple',description:'Flyer QA',category:'Supports imprimés',unit:'support',qty:1,unit_price:90,total:90}],subtotal:90,discount_amount:0,gross_total:90,deduction_amount:0,total:27,paid_amount:15,issue_date:'2026-09-25',service_date:'2026-09-25',purchase_order_reference:'BC-QA',due_date:'2026-10-02',notes:'Conditions QA'},{id:'i2',reference:'FAC-QA-002',kind:'Facture',status:'Payée',client_id:'c1',project_id:'pr2',quote_id:'q2',items:[{id:'li2',label:'Livrable final',description:'QA',category:'Digital',unit:'prestation',qty:1,unit_price:100,total:100}],subtotal:100,discount_amount:0,gross_total:100,deduction_amount:0,total:100,paid_amount:100,issue_date:'2026-09-25',service_date:'2026-09-25',purchase_order_reference:'BC-QA',due_date:'2026-10-02',notes:'Payé'}],
 payments:[{id:'pay1',invoice_id:'i1',amount:15,paid_at:'2026-09-25T09:00:00Z',method:'Virement',reference:'VIR-QA',notes:''},{id:'pay2',invoice_id:'i2',amount:100,paid_at:'2026-09-25T09:05:00Z',method:'Virement',reference:'VIR-PAID',notes:''}],
 businessProfile:{display_name:'JLG Creative',publisher:'Julien Mercadier',legal_form:'EI',siren:'123456789',siret:'12345678900011',address:'1 rue QA',postal_code:'19000',city:'Brive',country:'France',email:'qa@example.com',phone:'0500000000',vat_number:'',vat_note:'TVA non applicable, art. 293 B du CGI',mediator_name:'',mediator_url:'',operation_type:'Prestations de services',payment_terms_note:'Paiement à échéance.',early_payment_discount_note:'Escompte : néant.',late_payment_note:'Indemnité forfaitaire de 40 € pour clients professionnels.'},
...
// CREATIVE DESKTOP LIVE
{
 ...
 await check('Creative legal page has production wording',async()=>{const r=await page.request.get(CREATIVE+'legal.html');const t=await r.text();assert(/Mentions légales & confidentialité/i.test(t),'title');assert(!/À compléter avant lancement commercial massif/i.test(t),'old placeholder remains');assert(/Données personnelles/i.test(t),'privacy');assert(/professionnels et organisations/i.test(t),'professional-only scope')});
 ...
 await check('Creative clearly excludes private-consumer orders',async()=>{const t=await page.locator('body').innerText();assert(/Réservé aux professionnels/i.test(t),'scope badge');assert(/commandes de particuliers.*ne sont pas acceptées/i.test(t),'private consumer exclusion')});
 ...
 await check('Creative standard order validation and professional gate',async()=>{await page.locator('.featuredCard [data-order]').first().click();await page.locator('#next').click();assert(/Nom, structure/i.test(await page.locator('#toast').innerText()),'identity validation');await page.locator('[name=name]').fill('Test QA');await page.locator('[name=company]').fill('Société QA');await page.locator('[name=email]').fill('qa@example.com');assert(await page.locator('[name=professionalConfirmed]').count()===1,'professional confirmation');await page.locator('#next').click();assert(/Confirmez.*cadre professionnel/i.test(await page.locator('#toast').innerText()),'professional gate');await page.locator('[name=professionalConfirmed]').check();await page.locator('#next').click();assert((await page.locator('.orderModal').innerText()).includes('Votre besoin'),'step2');await page.locator('.orderModal .close').click()});
 ...
 await check('Creative poster wizard options, professional gate and A5 total',async()=>{await page.locator('[data-poster-order]').click();await page.locator('[name=name]').fill('Test QA');await page.locator('[name=company]').fill('Club QA');await page.locator('[name=email]').fill('qa@example.com');await page.locator('#next').click();assert(/Confirmez.*cadre professionnel/i.test(await page.locator('#toast').innerText()),'poster professional gate');await page.locator('[name=professionalConfirmed]').check();await page.locator('#next').click();assert(await page.locator('[name=format] option').count()===4,'formats');assert(await page.locator('[name=orientation] option').count()===2,'orientation');assert(await page.locator('[name=style] option').count()===11,'styles');await page.locator('[name=subject]').fill('Toulon');await page.locator('[name=format]').selectOption('A5');await page.locator('#next').click();const t=await page.locator('.posterSummary').innerText();assert(t.replace(/\u00a0/g,' ').includes('4,90 €'),'price');assert(t.includes('A5'),'format');await page.locator('.orderModal .close').click()});
...
// STUDIO LOGIN LIVE
{
 ...
 await check('Studio HTTP 200 and assets',async()=>{for(const p of ['manifest.webmanifest','sw.js','assets/styles.css','assets/studio.js','assets/pwa.js','assets/icon-192.png']){const r=await page.request.get(STUDIO+p);assert(r.ok(),p+' '+r.status())}});
 await check('Studio PWA is installable configuration',async()=>{const r=await page.request.get(STUDIO+'manifest.webmanifest');const m=await r.json();assert(m.display==='standalone','display '+m.display);assert(String(m.start_url||'').length>0,'start_url');assert((m.icons||[]).some(x=>x.sizes==='192x192'),'192 icon');assert((m.icons||[]).some(x=>x.sizes==='512x512'),'512 icon');assert(await page.locator('[data-install-app]').count()>=1,'install control missing')});
...
// STUDIO AUTHENTICATED FRONTEND WITH MOCKED API
{
 ...
 await page.route(API+'**',async route=>{
   ...
   if(a==='record-payment')return json({payment:{id:'pay3'},invoice:{...fakeData.invoices[0],paid_amount:27,status:'Payée'},remaining:0});
   if(a==='save-delivery')return json({delivery:{id:'dqa',project_id:'pr2',title:'Livraison finale',version:'V1',status:'Disponible',url:'https://example.com/final.zip',delivery_date:'2026-09-25'},payment_gate:{blocked:false,outstanding:0}});
 ...
 const views=[['orders','Demandes clients'],['packs','Toutes mes offres'],['clients','Clients'],['projects','Projets'],['tasks','Production'],['quotes','Devis'],['invoices','Finance'],['deliveries','Livraisons'],['settings','Réglages'],['dashboard','Tableau de bord']];
 ...
 await check('Studio Finance supports invoice type, details and partial payment capture',async()=>{await page.locator('#createRecord').click();assert(await page.locator('.adminModal [name=kind]').count()===1,'kind');assert(await page.locator('.adminModal [name=total]').count()===1,'total');assert(await page.locator('.adminModal [name=service_date]').count()===1,'service date');assert(await page.locator('.adminModal [name=purchase_order_reference]').count()===1,'purchase order');assert(await page.locator('.adminModal [name=paid_amount]').count()===0,'paid_amount must not be edited directly');await page.locator('.adminModal .close').click();await page.locator('[data-detailinvoice=i1]').click();const detail=await page.locator('.adminModal').innerText();assert(/15/.test(detail),'payment total');assert(detail.includes('Virement'),'payment history');await page.locator('.adminModal .close').click();await page.locator('[data-paidinvoice=i1]').click();assert(await page.locator('.adminModal [name=amount]').count()===1,'amount');assert(await page.locator('.adminModal [name=method]').count()===1,'method');await page.locator('.adminModal [name=amount]').fill('12');await page.locator('#savePayment').click();await page.waitForTimeout(80);assert(calls.some(x=>x.action==='record-payment'&&x.method==='POST'),'payment API');}); await clearOverlays(page);
 await page.locator('[data-view=deliveries]').click();
 await check('Studio delivery gate blocks unpaid work and allows paid projects',async()=>{await page.locator('#createRecord').click();const project=page.locator('.adminModal [name=project_id]'),status=page.locator('.adminModal [name=status]');await project.selectOption('pr1');await status.selectOption('Disponible');assert((await page.locator('#deliveryGate').innerText()).includes('bloquée')||(await page.locator('#deliveryGate').innerText()).includes('Aucun devis accepté'),'unpaid gate');assert(!(await page.locator('#overridePayment').isHidden()),'override unavailable');await project.selectOption('pr2');assert((await page.locator('#deliveryGate').innerText()).includes('Paiement OK'),'paid gate');await page.locator('.adminModal [name=title]').fill('Livraison finale');await page.locator('.adminModal [name=url]').fill('https://example.com/final.zip');await page.locator('#saveDelivery').click();await page.waitForTimeout(80);assert(calls.some(x=>x.action==='save-delivery'&&x.method==='POST'),'delivery API')}); await clearOverlays(page);
 await clearOverlays(page); await page.locator('[data-view=settings]').click();
 await check('Studio Settings legal profile and password validation work',async()=>{assert(await page.locator('[data-biz=publisher]').count()===1,'publisher');assert(await page.locator('[data-biz=siret]').count()===1,'siret');assert(await page.locator('[data-biz=late_payment_note]').count()===1,'late payment terms');assert(await page.locator('#saveBusiness').count()===1,'save business');await page.locator('#newPassword').fill('short');await page.locator('#changePassword').click();assert((await page.locator('#studioToast').innerText()).includes('10 caractères'),'password validation')});
...
// STUDIO AUTHENTICATED MOBILE WITH MOCKED API
{
 ...
 await check('Studio mobile More menu exposes secondary sections and install button',async()=>{await page.locator('#mobileMore').click();await page.waitForTimeout(260);assert(await page.locator('#mobileMoreSheet').evaluate(e=>e.classList.contains('open')),'more not open');const install=page.locator('.mobileInstallAction');await install.scrollIntoViewIfNeeded();assert(await install.isVisible(),'install hidden');const ib=await install.boundingBox();assert(ib&&ib.y>=0&&ib.y+ib.height<=844,'install outside phone viewport');for(const k of ['packs','clients','tasks','invoices','deliveries','settings'])assert(await page.locator('[data-mobile-view='+k+']').count()===1,'missing '+k);await page.locator('[data-mobile-view=invoices]').click();assert((await page.locator('.studioTop h1').innerText()).includes('Finance'),'finance title')});
...
// CLIENT PORTAL WITH MOCKED API
{
 ...
 const portalData={
   ...
   client:{id:'c1',name:'Client QA',email:'qa@example.com',phone:'0600000000',address:'2 avenue QA',postal_code:'19100',city:'Brive',country:'France',siren:'987654321'},
   businessProfile:{display_name:'JLG Creative',publisher:'Julien Mercadier',legal_form:'EI',siret:'12345678900011',address:'1 rue QA',postal_code:'19000',city:'Brive',country:'France',email:'qa@example.com',phone:'0500000000',vat_note:'TVA non applicable, art. 293 B du CGI',operation_type:'Prestations de services',payment_terms_note:'Paiement à échéance.',early_payment_discount_note:'Escompte : néant.',late_payment_note:'Indemnité forfaitaire de 40 € pour clients professionnels.'},
 ...
 await check('Client sees complete quote before accepting',async()=>{const card=page.locator('[data-doc-card="quote-q1"]');assert(await card.count()===1,'quote card');await card.locator('summary').click();const t=await card.innerText();for(const x of ['Flyer simple','349','Acompte','Conditions QA','JLG Creative','Client QA'])assert(t.includes(x),'quote detail '+x)});
 await check('Client sees complete invoice and payment history',async()=>{const card=page.locator('[data-doc-card="invoice-i1"]');assert(await card.count()===1,'invoice card');await card.locator('summary').click();const t=await card.innerText();for(const x of ['Reste à payer','Historique des règlements','Virement','JLG Creative','Client QA','40 €'])assert(t.includes(x),'invoice detail '+x);assert(/acompte/i.test(t),'invoice detail Acompte')});
 ...
 await check('Client quote acceptance calls API',async()=>{page.once('dialog',d=>d.accept());await page.locator('[data-quote=q1][data-qstatus="Accepté"]').click();await page.waitForTimeout(80);assert(calls.some(x=>x.action==='portal-quote'&&x.method==='POST'&&String(x.body||'').includes('"professionalConfirmed":true')),'no confirmed professional quote call')});
