...
// CLIENT PORTAL WITH MOCKED API
{
 ...
 await check('Client quote acceptance calls API',async()=>{page.once('dialog',d=>d.accept());await page.locator('[data-quote=q1][data-qstatus="Accepté"]').click();await page.waitForTimeout(80);assert(calls.some(x=>x.action==='portal-quote'&&x.method==='POST'&&x.body?.professionalConfirmed===true),'no confirmed professional quote call')});
